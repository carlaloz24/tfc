...
class DietaController extends Controller
{
    public function getPdf(Request $request, $mascota_id)
    {
        try {
            ...
            Log::error('DietaController@getPdf: Error inesperado', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            Log::info('DietaController@store: Datos validados', ['validated' => $validated]);

            $mascota = Mascota::where('id', $request->mascota_id)
                ->where('id_usuario', Auth::id())
                ->select('id', 'nombre', 'peso', 'categoria_edad', 'esterilizado', 'nivel_actividad', 'tipo_dieta_preferida', 'condiciones_salud', 'alimentos_alergia')
                ->firstOrFail();

            Log::info('DietaController@store: Mascota cargada', ['mascota' => $mascota->toArray()]);
            ...
            // Decodificar menu_json
            $menu = json_decode($request->menu_json, true);
            Log::info('DietaController@store: Menu decodificado', ['menu' => $menu]);

            if (empty($menu)) {
                Log::warning('DietaController@store: Menu JSON vacío', ['mascota_id' => $mascota->id]);
                throw new \Exception('El menú semanal no se generó correctamente');
            }
            ...
            $pdf = PDF::loadView('calculadora.pdf', [
                ...
                'menu' => $menu,
                'condiciones_salud' => $request->condiciones_salud ?? $mascota->condiciones_salud ?? [],
                'alimentos_alergia' => $request->alimentos_alergia ?? $mascota->alimentos_alergia ?? [],
            ]);
            ...
            Log::info('DietaController@store: Dieta guardada', [
                ...
                'has_pdf_dieta' => !empty($dieta->pdf_dieta),
                'menu_json_length' => strlen($request->menu_json),
            ]);

            return redirect()->route('profile.index')->with('success', 'Dieta generada correctamente.');
        } catch (\Exception $e) {
            Log::error('DietaController@store: Error al guardar dieta', [
                'mascota_id' => $request->mascota_id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
            return back()->with('error', 'Error al generar la dieta: ' . $e->getMessage());
        }
    }
}
